...
//     // ✅ SAVE FILES
// const allFiles = [
//   ...(req.files.profilePhoto || []).map(f => ({ ...f, type: "profilePhoto" })),
//   ...(req.files.idProof || []).map(f => ({ ...f, type: "idProof" })),
//   ...(req.files.medicalReports || []).map(f => ({ ...f, type: "medicalReports" })),
//   ...(req.files.prescription || []).map(f => ({ ...f, type: "prescription" })),
// ];

// const values = allFiles.map(file => [
//   request_id,
//   file.path,
//   file.type
// ]);
...
//     db.query(timelineQuery, [id, label]);
...
// //   const query = `SELECT certificate_file FROM certificate_requests WHERE id = ?`;

// //   db.query(query, [id], (err, result) => {
// //     if (err) return res.status(500).json(err);

// //     const file = result[0]?.certificate_file;
...
exports.approveRequest = async (req, res) => {
 ...
 console.log("3");
   ...
   LIMIT 1`,
    ...
    // doc.text(`Patient Name: ${request.full_name}`);
    // doc.text(`Doctor Name: ${doctorName}`);
    // doc.text(`Clinic: ${clinicName}`);
    // doc.moveDown();

    // doc.text(`Certificate Type: ${request.certificate_type}`);
    // doc.text(`Purpose: ${request.purpose}`);
    // doc.text(`Medical Conditions: ${request.medical_conditions || "N/A"}`);
    // doc.moveDown();

    // doc.text(
    //   `DOB: ${request.dob ? new Date(request.dob).toLocaleDateString("en-IN") : "N/A"}`,
    // );
    // doc.text(`Gender: ${request.gender || "N/A"}`);
    // doc.moveDown();

    // doc.text(`Notes: ${request.notes || "Normal"}`);
    // doc.text(`Treatment: ${fitness_status || "N/A"}`);
    // doc.text(`Medicines: ${request.medications || "N/A"}`);
    // doc.moveDown();

    // doc.text(`Validity (days): ${validity || 0}`);
    // doc.text(
